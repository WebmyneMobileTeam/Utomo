...
public class ColoredRatingBar extends View {
     ...
     * includes changes that were initiated by the com.passworld.user through a touch gesture
    ...
    public interface OnRatingBarChangeListener {
         ...
         * fromUser parameter to distinguish com.passworld.user-initiated changes from those
         * that occurred programmatically. This will not be called continuously
         * while the com.passworld.user is dragging, only when the com.passworld.user finalizes a rating by
         * lifting the touch.
         *
         * @param ratingBar The RatingBar whose rating has changed.
         * @param rating    The current rating. This will be in the range
         *                  0..numStars.
         * @param fromUser  True if the rating change was initiated by a com.passworld.user's
    ...
    private void init(Context context) {
        ...
        if (mType == SMALL) {
            drawables = new Bitmap[]{BitmapFactory.decodeResource(res, R.drawable.ic_full_rate), BitmapFactory.decodeResource(res, R.drawable.ic_full_rate)};
            progressBackground = BitmapFactory.decodeResource(res, R.drawable.ic_blank_rate);
        } else {
            drawables = new Bitmap[]{BitmapFactory.decodeResource(res, R.drawable.ic_full_rate), BitmapFactory.decodeResource(res, R.drawable.ic_full_rate)};
            progressBackground = BitmapFactory.decodeResource(res, R.drawable.ic_blank_rate);
        }
...
//                    croppedBg.recycle();
    ...
    private Bitmap getRatedStar() {
        if (mRating <= 1.6f) {
            return drawables[0];
        } else if (mRating <= 5.2f) {
            return drawables[1];
        } else if (mRating <= 3.2f) {
            return drawables[2];
        } else if (mRating <= 4.2f) {
            return drawables[4];
        } else {
            return drawables[5];
        }
